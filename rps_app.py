"""
✌️✊🖐️ 가위바위보 독심술 & AI 카메라 - Streamlit 버전
-------------------------------------------------
로컬 실행:
...
# 기본 여백 정돈 및 Streamlit 탭 디자인 시각화
st.markdown(
    """
    <style>
...
st.title("✂️🪨📄 가위바위보 종합 대결")

# 탭 이름 생성
tab_mindreader, tab_camera = st.tabs([
    "🧠 [모드 1] 독심술 패턴 분석 대결", 
    "📸 [모드 2] AI 카메라/사진 인식 대결"
])

# =========================================================================
# 탭 1: rps_app.py 독심술 가위바위보 게임
# =========================================================================
with tab_mindreader:
    GAME_HTML = """
    <!DOCTYPE html>
    ...
    *{box-sizing:border-box}
    ...
    h1{font-size:clamp(24px,4vw,36px);margin:0;line-height:1.1}
    ...
    """
    components.html(GAME_HTML, height=1350, scrolling=True)

# =========================================================================
# 탭 2: app3.py 카메라/파일 업로드 AI 인식 대결 모드
# =========================================================================
with tab_camera:
    st.subheader("📸 AI 카메라 인식 대결 (Teachable Machine)")
    st.write(
        "카메라 촬영이나 이미지 업로드를 이용해 Teachable Machine AI가 인식한 손 모양으로 대결합니다."
    )
    ...
    input_method = st.radio(
        "입력 방식 선택",
        ["📸 카메라로 바로 촬영하기", "📁 파일 업로드하기"],
        horizontal=True,
        key="rps_input_method",
    )

    if input_method.startswith("📸"):
        uploaded_hand = st.camera_input("가위/바위/보 손 모양을 카메라로 촬영하세요", key="rps_camera")
    else:
        uploaded_hand = st.file_uploader("손 모양 사진 선택 (PNG, JPG)", type=["png", "jpg", "jpeg"], key="rps_upload")
    ...
    if model_url and uploaded_hand:
        ...
        with col_left:
            st.image(image, caption="촬영한 손 모양", use_container_width=True)

        with col_right:
            html_code = """
            <div style="font-family:-apple-system, sans-serif;">
              <div id="status" style="color:#555; margin-bottom:10px; font-weight:bold;">AI가 손 모양을 분석 중입니다... ⏳</div>
            ...
            <script type="text/javascript">
              ...
              function renderBar(className, prob) {
                const normName = normalize(className);
                const pct = (prob * 100).toFixed(1);
                return `<div style="margin-bottom:6px;">
                          <div style="display:flex; justify-content:space-between; font-size:13px;">
                            <span>${EMOJI[normName] || ""} ${normName}</span><span>${pct}%</span>
                          </div>
                ...
                try {
                  ...
                  imgEl.onload = async () => {
                    ...
                    if (BEATS[player] === undefined) {
                      statusEl.innerText = "⚠️ 인식 실패";
                      resultBox.innerHTML =
                        "인식된 클래스 이름이 '가위/바위/보'와 일치하지 않습니다.";
                      return;
                    }
                    ...
                    if (player === COMPUTER_CHOICE) {
                      outcome = "🤝 무승부!";
                      outcomeColor = "#6c757d";
                    } else if (BEATS[player] === COMPUTER_CHOICE) {
                      outcome = "🎉 내가 이겼다!";
                      outcomeColor = "#28a745";
                    } else {
                      outcome = "😢 내가 졌다!";
                    ...
                    // 직관적으로 '내가 낸 거'와 '컴퓨터가 낸 거' 표기
                    resultBox.innerHTML = `
                      <div style="background-color:#f8f9fa; border:2px solid #e9ecef; border-radius:12px; padding:15px; text-align:center;">
                        <div style="display:flex; justify-content:space-around; align-items:center; margin-bottom:10px;">
                          <div>
                            <span style="font-size:12px; color:#6c757d; display:block;">내가 낸 거</span>
                            <span style="font-size:32px;">${EMOJI[player] || ""}</span>
                            <b style="font-size:18px; display:block;">${player}</b>
                          </div>
                          <div style="font-size:20px; font-weight:bold; color:#adb5bd;">VS</div>
                          <div>
                            <span style="font-size:12px; color:#6c757d; display:block;">컴퓨터가 낸 거</span>
                            <span style="font-size:32px;">${EMOJI[COMPUTER_CHOICE]}</span>
                            <b style="font-size:18px; display:block;">${COMPUTER_CHOICE}</b>
                          </div>
                        </div>
                        <div style="font-size:22px; font-weight:bold; color:${outcomeColor}; margin-top:10px; border-top:1px dashed #dee2e6; padding-top:10px;">
                          ${outcome}
                        </div>
                      </div>
                    `;
                  ...
                  statusEl.innerHTML =
                    "❌ 모델을 불러올 수 없습니다. Teachable Machine URL을 확인해 주세요.<br><small>" + err + "</small>";
                }
              }
              init();
            </script>
        ...
        st.info("카메라 촬영이나 이미지 업로드를 완료하면 컴퓨터와 자동으로 대결이 시작됩니다.")